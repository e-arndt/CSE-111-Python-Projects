...
def populate_main_window(frm_main):
    ...
    lbl_age = Label(frm_main, text="Weight (10 - 500):")

    # Create an integer entry box where the user will enter her age.
    ent_age = IntEntry(frm_main, width=4, lower_bound=10, upper_bound=500)

    # Create a label that displays "years"
    lbl_age_units = Label(frm_main, text="pounds")

    # Create a label that displays "Rates:"
    lbl_rates = Label(frm_main, text="Earth")
    lbl_rates2 = Label(frm_main, text="Moon")

    # Create labels that will display the results.
    lbl_slow = Label(frm_main, width=3)
    lbl_fast = Label(frm_main, width=3)
    lbl_rate_units = Label(frm_main, text="Weight On:")
    ...
    lbl_rates.grid(     row=2, column=1, padx=(3,3), pady=3)
    lbl_rates2.grid(    row=2, column=2, padx=(3,3), pady=3)
    lbl_slow.grid(      row=1, column=1, padx=3, pady=3)
    lbl_fast.grid(      row=1, column=2, padx=3, pady=3)
    lbl_rate_units.grid(row=1, column=0, padx=0, pady=3)
